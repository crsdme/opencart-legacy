...
class ControllerCommonMicrodata extends Controller
{
	public function index($data)
	{
		...
		$blocks[] = $this->faqSchema($data, $context);

		if ($route === 'blog/article') {
			$blocks[] = $this->blogArticleSchema($data, $context);
		}

		if ($route === 'blog/author') {
			$blocks[] = $this->blogAuthorSchema($data, $context);
		}

		$html = '';

		foreach ($blocks as $block) {
			if (!is_array($block) || empty($block['json'])) {
				continue;
			}
	...
	private function faqSchema($data, $context)
	{
		if (empty($data['faq']) || empty($data['faq']['items']) || !is_array($data['faq']['items'])) {
			return null;
		}
		...
		foreach ($data['faq']['items'] as $item) {
			$name = isset($item['title']) ? $this->plainText($item['title']) : '';
			$text = isset($item['text']) ? $this->plainText($item['text']) : '';
		...
		return $this->jsonLdBlock('FAQPage JSON-LD', [
			'@context' => 'https://schema.org',
			'@type' => 'FAQPage',
			'@id' => $context['url'] . '#faq',
			'inLanguage' => $context['language'],
			'name' => !empty($data['faq']['title'])
				? $this->plainText($data['faq']['title'])
				: $this->plainText($context['title']),
			'url' => $context['url'],
			'mainEntity' => $questions,
		]);
	}

	private function blogArticleSchema($data, $context)
	{
		$article_id = (int) ($this->request->get['article_id'] ?? 0);

		if (!$article_id) {
			return null;
		}

		$headline = $this->plainText($data['heading_title'] ?? $context['title']);
		$url = $this->url->link('blog/article', 'article_id=' . $article_id);
		$image = '';

		if (!empty($data['gallery'][0]['popup'])) {
			$image = $data['gallery'][0]['popup'];
		...
		$schema = [
			...
			'dateModified' => $data['date_modified'] ?? ($data['date_published'] ?? null),
			'image' => $image ?: null,
			'publisher' => [
				'@id' => $context['store_url'] . '#organization',
			],
			'author' => $this->personNode($data['author'] ?? [], $context),
		];

		return $this->jsonLdBlock('Article JSON-LD', $schema);
	}

	private function blogAuthorSchema($data, $context)
	{
		if (empty($data['author']) || !is_array($data['author'])) {
			return null;
		}

		$person = $this->personNode($data['author'], $context, true);

		if (!$person) {
			return null;
		}

		$url = html_entity_decode((string) ($data['author']['href'] ?? $context['url']), ENT_QUOTES, 'UTF-8');
		$person['@context'] = 'https://schema.org';
		$person['@id'] = $url . '#person';
		$person['mainEntityOfPage'] = html_entity_decode((string) $context['url'], ENT_QUOTES, 'UTF-8');
		$person['worksFor'] = [
			'@id' => $context['store_url'] . '#organization',
		];

		return $this->jsonLdBlock('Person JSON-LD', $person);
	}

	private function personNode($author, $context, $full = false)
	{
		...
		$person = [
			'@type' => 'Person',
			'name' => $this->plainText($author['name']),
			'url' => !empty($author['href'])
				? html_entity_decode($author['href'], ENT_QUOTES, 'UTF-8')
				: null,
			'image' => $author['thumb'] ?? ($author['image'] ?? null),
		];

		if ($full) {
			$description = $this->plainText($author['meta_description'] ?? '');

			if ($description === '' && !empty($author['description'])) {
				$description = $this->plainText($author['description']);
			}

			$person['description'] = $description ?: null;
		}
