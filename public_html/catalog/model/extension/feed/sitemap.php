...
class ModelExtensionFeedSitemap extends Model
{
	public function getCategories(): array
	{
		...
		if ($catHasNoindex && $this->indexedOnly()) {
			$sql .= " AND c.noindex = '1'";
		}
	...
	public function getProducts(): array
	{
		...
		if ($prodHasNoindex && $this->indexedOnly()) {
			$sql .= " AND p.noindex = '1'";
		}
	...
	public function getInformation(): array
	{
		...
		if ($infoHasNoindex && $this->indexedOnly()) {
			$sql .= " AND i.noindex = '1'";
		}
	...
	public function getManufacturers(): array
	{
		...
		if ($manHasNoindex && $this->indexedOnly()) {
			$sql .= " AND m.noindex = '1'";
		}
	...
	public function getBlogAuthors(): array
	{
		$sql =
			'SELECT a.author_id, ad.name FROM ' .
			DB_PREFIX .
			'blog_author a LEFT JOIN ' .
			DB_PREFIX .
			'blog_author_description ad ON (a.author_id = ad.author_id)';
		...
		if ($this->indexedOnly()) {
			$noindex = $this->db->query("SHOW COLUMNS FROM `" . DB_PREFIX . "blog_author` LIKE 'noindex'");
		...
		$sql .= ' ORDER BY a.sort_order ASC, LCASE(ad.name)';

		$q = $this->db->query($sql);
		return $q->rows;
	}

	private function useMultishop()
	{
		return (int) $this->config->get('feed_sitemap_multishop') === 1;
	}

	private function indexedOnly()
	{
		$value = $this->config->get('feed_sitemap_indexed_only');
