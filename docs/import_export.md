...
**CSV** — one entity per file. Id columns first. Localized columns are `name.uk`. `category_ids` uses `|` (`12|15` or `-1|-2`). `attributes` is a JSON cell with `attribute_id`.

Images: local paths under `image/catalog/...`, or `http(s)` URLs. Remote files are downloaded on import (not preview) into `image/catalog/import/`, hashed by URL so the same link is fetched once. JPEG, PNG, GIF and WebP, max 8 MB. Private/local hosts are refused. A failed download skips that image and keeps the previous one on update. Turn this off in **Settings** if you only want local paths.
